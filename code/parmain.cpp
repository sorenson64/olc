...
int main(int argc, char *argv[])
{
  ...
  char filename[]="xxx.out";
  filename[0]=(char)(id/100+'0');
  filename[1]=(char)((id/10)%10+'0');
  filename[2]=(char)(id%10+'0');
...
const uint64_t nchunk=1e9;
for(uint64_t myn=1e10+id*nchunk; myn<1e12; myn+=np*nchunk)
{
  int64_t n=(int64_t)myn; // start point
  nbegin=n;
